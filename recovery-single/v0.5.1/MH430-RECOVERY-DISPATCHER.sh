...
DP18_WRAPPER_REF="651daba319b0b73afec72a78eec8d92f347cc352"
DP18_WRAPPER_SHA="13b9cdbb813f59c2c3abd9c884cda87f9f27d4ad185aac769215d5c425f98110"
DP18_WRAPPER_URL="https://raw.githubusercontent.com/MicrohardAssistenza/dp18-recovery/$DP18_WRAPPER_REF/recovery-single/v0.5.0/DP18-RECOVERY-V114-WRAPPER.sh"
...
install_mode(){
  ...
  # Rimuovi i loader universali obsoleti. Il lean recovery li ricrea se serve.
...
resume_mode(){
  ...
  case "$class" in
    DDX)
      ...
      ;;
    DP18)
      tmp="/root/DP18-RECOVERY-V114-WRAPPER.sh.new"
      say "ROUTE $id -> DP18 FULL RECOVERY v1.14 (v1.13 validata + registry opzionale)"
      curl -k -fL --retry 5 --retry-delay 2 --connect-timeout 15 --max-time 60 "$DP18_WRAPPER_URL" -o "$tmp"
      [ "$(sha256sum "$tmp" | awk '{print $1}')" = "$DP18_WRAPPER_SHA" ] || { say 'ERRORE SHA DP18 wrapper v1.14'; exit 31; }
      bash -n "$tmp"
      mv -f "$tmp" /root/DP18-RECOVERY-V114-WRAPPER.sh; chmod 700 /root/DP18-RECOVERY-V114-WRAPPER.sh
      cleanup_dispatch_service
      say "HANDOFF DP18 wrapper v1.14"
      exec env DP18_SFTP_PASSWORD="$sftp" DP18_GITHUB_TOKEN="$token" /root/DP18-RECOVERY-V114-WRAPPER.sh
      ;;
    LEGACY)
