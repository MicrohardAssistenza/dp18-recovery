...
write_summary(){
  result="$1"; pp_rc="${2:-NA}"; read_counts
...
run_test(){
  ...
  if [ ! -f "$F" ]; then
    say "ERRORE: $F assente"; write_summary "NO_MHA" "NA"; finish_logger; exit 0
  fi
  ...
  if [ "$ACTUAL" != "$EXPECTED_SHA" ]; then
    ...
    write_summary "WRONG_MHA_$ACTUAL" "NA"; finish_logger; exit 0
  fi

  say "Fermo paypoint.service e libero /dev/ttyAMA0"
  systemctl stop paypoint.service >/dev/null 2>&1 || true
  sleep 1
  if command -v stdbuf >/dev/null 2>&1; then PP_CMD=(stdbuf -oL -eL /root/paypointserver); else PP_CMD=(/root/paypointserver); fi

  say "Avvio paypointserver locale con output persistente"
  ( sleep 2; say ">>> TRIGGER update-ready <<<"; rm -f /tmp/uploads/update-ready; touch /tmp/uploads/update-ready; sync ) &

  set +e
  if command -v timeout >/dev/null 2>&1; then
    timeout 120 "${PP_CMD[@]}"; PP_RC=$?
  else
    "${PP_CMD[@]}" & P=$!; sleep 120; kill "$P" >/dev/null 2>&1 || true; wait "$P" >/dev/null 2>&1; PP_RC=$?
  fi
  set -e
...
status(){
  ...
  echo "=== LOG TAIL ==="; tail -120 "$LOG" 2>/dev/null || true
}
